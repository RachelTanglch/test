...
#1、NE2的vpls域中创建acPwCnt-1个ac，1个到NE1的pw，ac从接口vlan10递增（raw p+v）
#   NE1的vpls域中创建acPwCnt-1个ac，1个到NE2的pw，ac从接口vlan10递增（raw p+v）
#   NE1上使能每个lsp、pw、ac的性能统计
#2、NE1和NE2都发送smac、dmac、vid（vid从10开始到acPwCnt-2）递增的数据流双向各
#   600M，字节长度为random，len从64-1518；NE1和NE3上发送smac变化（vid从10开始到
#   acPwCnt-2）递增的arp
#预期结果：（1）每条流都能正常转发，只检查是否收到每条数据流不检查丢包
#（2）查看NE1 lsp ac pw的性能统计并把结果打印到long中，不判断性能统计是否正确
#
#3、读取系统资源（show system resource usage）反复shutdown/undo shut NE3板卡的
#   NNI口和UNI口（次数可以配置），每次shut undoshut后检查每条流的转发，只检查
#   是否收到每条数据流不检查丢包；查看NE1 lsp ac pw的性能统计并把结果打印到long
#   中，不判断性能统计是否正确
#预期结果：每次shut undoshut后，每条流都能正常转发（2）cpu的利用率误差在5%，用户
#    利用率的误差在1%，系统利用率的误差在1%。查看NE1 lsp ac pw的性能统计并把结果
#    打印到long中，不判断性能统计是否正确
...
if {[catch {
	close stdout
	file mkdir "log"
	set fd_log [open "log\\GPN_PTN_009_2_LOG.txt" a]
	set stdout $fd_log
	chan configure $fd_log -blocking 0 -buffering none;#非阻塞模式 按行flush
	log_file log\\GPN_PTN_009_2_LOG.txt
	
	file mkdir "report"
	set fileId [open "report\\GPN_PTN_009_2_REPORT.txt" a+]
	chan configure $fileId -blocking 0 -buffering none;#非阻塞模式 按行flush
	
	file mkdir "debug"
	set fd_debug [open debug\\GPN_PTN_009_2_DEBUG.txt a]
	exp_internal -f debug\\GPN_PTN_009_2_DEBUG.txt 0
   ...
   ###配置端口1
    ...
    foreach hStream "$dStreamData16 $dStreamData26" {
  ...
  gwd::Sub_TxAndRx_Info $hProject $hGPNPort4 rxInfoArr2 hGPNPort4RxInfo2
    ...
    incr tcId 
    gwd::GWpublic_saveTCCfg 1 $fd_log "GPN_LSP_009_2_Case1_$tcId.xml" [pwd]/Untitled
    puts $fileId ""
    puts $fileId "======================================================================================\n"
    ...
    set cfgFlag 0
    set flag1_Case1 0;set flag2_Case1 0;set flag3_Case1 0;set flag4_Case1 0;set flag5_Case1 0;set flag6_Case1 0;set flag7_Case1 0;set flag8_Case1 0;set flag9_Case1 0
    set pw12 "pw12_1"
    set pw21 "pw21_1"
	...
	"E-LAN容量及稳定性测试基础配置成功，继续后面的测试" "GPN_PTN_009_2"
    puts $fileId ""
	...
	###配置vpls
	lappend flag1_Case1 [gwd::GWVpls_AddInfo $telnet1 $matchType1 $Gpn_type1 $fileId "vpls1" 1 "elan" "flood" "false" "false" "false" "true" "5000" "" "tagged"]
	lappend flag1_Case1 [gwd::GWVpls_AddInfo $telnet2 $matchType2 $Gpn_type2 $fileId "vpls1" 1 "elan" "flood" "false" "false" "false" "true" "5000" "" "tagged"]
    ####配置NE1
	...
	lappend flag1_Case1 [gwd::GWpublic_CfgPw $telnet2 $matchType2 $Gpn_type2 $fileId "$pw21" "vpls" "vpls1" "peer" $address621 "100" "100" "" "add" "none" 1 0 "0x8100" "0x8100" "1"]
	####配置lsp、ac、pw性能统计使能
	...
	lappend flag1_Case1 [gwd::GWAc_AddVplsInfo $telnet1 $matchType1 $Gpn_type1 $fileId "ac1000" "vpls1" "ethernet" $GPNTestEth1 "1000" "0" "root" "modify" "1000" "0" "0" "0x8100" "evc2"]	
	lappend flag1_Case1 [gwd::GWAc_AddVplsInfo $telnet2 $matchType2 $Gpn_type2 $fileId "ac1000" "vpls1" "ethernet" $GPNTestEth2 "1000" "0" "root" "modify" "1000" "0" "0" "0x8100" "evc2"]
	...
	for {set i 1} {$i < [expr $acPwCnt-1]} {incr i} {
		...
		lappend flag1_Case1 [PTN_UNI_AddInter $telnet1 $matchType1 $Gpn_type1 $fileId $Worklevel $acVlan $GPNTestEth1]
		lappend flag1_Case1 [PTN_UNI_AddInter $telnet2 $matchType2 $Gpn_type2 $fileId $Worklevel $acVlan $GPNTestEth2]
		lappend flag1_Case1 [gwd::GWAc_AddVplsInfo $telnet1 $matchType1 $Gpn_type1 $fileId "ac$acVlan" "vpls1" "ethernet" $GPNTestEth1 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" "evc2"]	
		lappend flag1_Case1 [gwd::GWAc_AddVplsInfo $telnet2 $matchType2 $Gpn_type2 $fileId "ac$acVlan" "vpls1" "ethernet" $GPNTestEth2 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" "evc2"]
		lappend flag1_Case1 [gwd::GWpublic_addAcStatEn $telnet1 $matchType1 $Gpn_type1 $fileId "ac$acVlan" "enable"]
  ...
  gwd::GW_AddUploadFile $telnet1 $matchType1 $Gpn_type1 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
    "config" "GPN_PTN_009_2_fullacNE1.txt" "" "120"
  ...
  gwd::GW_AddUploadFile $telnet2 $matchType2 $Gpn_type2 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
    "config" "GPN_PTN_009_2_fullacNE2.txt" "" "120"
  incr cfgId
  lappend FLAGF [gwd::GWpublic_uploadDevCfg 1 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp]
  if {$lFailFileTmp != ""} {
    set lFailFile [concat $lFailFile $lFailFileTmp]
  }
    if {"1" in $flag1_Case1} {
	...
	lappend flag2_Case1 [MPLS_ClassStatisticsAna "1" $infoObj1 $infoObj2 "1000" "00:00:00:01:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuecnt1 valuecnt2 "GPN_PTN_009_2_A2_1"]
   ...
   foreach hStream "$dStreamData16 $dStreamData26" {
    ...
    incr tcId 
    gwd::GWpublic_saveTCCfg 1 $fd_log "GPN_LSP_009_2_Case1_$tcId.xml" [pwd]/Untitled
    lappend flag2_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_FA2"]
  ...
  puts $fileId ""
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	if {"1" in $flag2_Case1} {
		set flagCase1 1
	...
	puts $fileId "======================================================================================\n"
	gwd::GWpublic_sendSameStrToFiles "$fd_debug $fd_log $fileId" "=====满配ac时，NE1($gpnIp1)反复shutdown/undo shut NE3板卡的NNI口和UNI口，测试业务恢复和系统内存   测试开始=====\n"
	...
	foreach eth "$GPNPort5 $GPNTestEth1" {
		...
		for {set j 1} {$j<=$cnt} {incr j} {
			...
			after 120000
			stc::perform ResultsClearAll -PortList $lport1
            lappend flag3_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_FA3"]
  ...
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	...
	puts $fileId ""
	gwd::GWpublic_sendSameStrToFiles "$fd_debug $fd_log $fileId" "=====满配ac时，NE1($gpnIp1)反复shutdown/undo shut NE3板卡的NNI口和UNI口，测试业务恢复和系统内存  测试结束=====\n"
	puts $fileId "======================================================================================\n"
	...
	foreach slot $rebootSlotlist2 {
		...
		for {set j 1} {$j<=$cnt} {incr j} {
			if {![gwd::GWCard_AddReboot $telnet2 $matchType2 $Gpn_type2 $fileId $slot]} {
                ...
                lappend flag4_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A4_$j"]
  ...
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitchSw $telnet1 $matchType1 $Gpn_type1 $fileId]} {
            ...
            lappend flag5_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A5_$j"]
		} else {
               set testFlag 1
  ...
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitch $telnet1 $matchType1 $Gpn_type1 $fileId]} {
            ...
            set lSpawn_id [lreplace $lSpawn_id 1 1 $telnet2]
            lappend flag6_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A6_$j"]
		} else {
               set testFlag 1
  ...
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	...
	for {set j 1} {$j<=$cnt} {incr j} {
		...
		after [expr 2*$rebootTime]
       	set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
        set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
        lappend flag7_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A7_$j"]
	...
	stc::perform ResultsClearAll -PortList $lport1
	for {set j 1} {$j<=$cnt} {incr j} {
		lappend flag8_Case1 [gwd::GWmanage_GetSystemResource $telnet1 $matchType1 $Gpn_type1 $fileId "usage" resource6_1]
		lappend flag8_Case1 [gwd::GWStaPw_DelPw $telnet1 $matchType1 $Gpn_type1 $fileId $pw12]
		if {[expr $acPwCnt]<11} {
			for {set k 1} {$k<[expr $acPwCnt]} {incr k} {
			lappend flag8_Case1 [gwd::GWAc_DelName $telnet1 $matchType1 $Gpn_type1 $fileId "ac[expr $k+999]"]
			}
		} else {
		...
		lappend flag8_Case1 [gwd::GWpublic_CfgPw $telnet1 $matchType1 $Gpn_type1 $fileId "$pw12" "vpls" "vpls1" "peer" $address612 "100" "100" "" "add" "none" 1 0 "0x8100" "0x8100" "1"]
		lappend flag8_Case1 [gwd::GWpublic_addPwStatEn $telnet1 $matchType1 $Gpn_type1 $fileId "$pw12" "enable"]
      ...
      lappend flag8_Case1 [PTN_UNI_AddInter $telnet1 $matchType1 $Gpn_type1 $fileId $Worklevel $acVlan $GPNTestEth1]
    	...
    	lappend flag8_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A7_$j"]
  ...
  if {[string match "" $tmpresult2]} {
	...
	gwd::GWmanage_ftpDownload $telnet1 $matchType1 $Gpn_type1 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_009_2_fullacNE1.txt" ""
	gwd::GWmanage_ftpDownload $telnet2 $matchType2 $Gpn_type2 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_009_2_fullacNE2.txt" ""
	gwd::GWpublic_Reboot $telnet1 $matchType1 $Gpn_type1 $fileId
	gwd::GWpublic_Reboot $telnet2 $matchType2 $Gpn_type2 $fileId
	after [expr 2*$rebootTime]
	set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	...
	set telnet2 [gwd::GWpublic_loginGpn $gpnIp2 $userName2 $password2 $matchType2 $Gpn_type2 $fileId]
	set lSpawn_id [lreplace $lSpawn_id 1 1 $telnet2]
	lappend flag9_Case1 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_009_2_A7_$j"]
  ...
  gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_2" lFailFileTmp
	...
	after $rebootTime
	gwd::GWpublic_printAbnormal $fileId $fd_log $flagDel "FLAGD" $startSeconds "测试结束后配置恢复" "测试结束后配置恢复" "GPN_PTN_009_2"
	gwd::GWpublic_printAbnormal $fileId $fd_log $FLAGF "FLAGF" $startSeconds $lFailFile "测试过程中所有配置文件都上传成功" "GPN_PTN_009_2"
	chan seek $fileId 0
	puts $fileId "\n**************************************************************************************\n"
	puts $fileId "**************************************************************************************\n"
	puts $fileId "GPN_PTN_009_2测试目的：E-LAN容量及稳定性测试\n"
	gwd::GWpublic_printCompletedRunTime $fileId $startSeconds
	if {$flagCase1==1 || [regexp {[^0\s]} $flagDel]} {
		set flagResult 1
	}
	gwd::GWpublic_print [expr {($flagResult == 1) ? "NOK" : "OK"}] "GPN_PTN_009_2测试结果" $fileId
	puts $fileId ""
	...
	gwd::GWpublic_print "== FA3 == [expr {($flag3_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，NE1($gpnIp1)反复shutdown/undo shut NE1板卡的NNI口和UNI口,测试对elan业务和CPU利用率的影响,[expr {($flag3_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA4 == [expr {($flag4_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，NE1($gpnIp1)反复重启NNI口所在板卡后,测试对elan业务和CPU利用率的影响,[expr {($flag4_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA5 == [expr {($flag5_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，NE1($gpnIp1)反复进行交换盘的倒换后,测试对ELAN业务和系统CPU利用率的影响,[expr {($flag5_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA6 == [expr {($flag6_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，NE1($gpnIp1)反复进行NMS的主备倒换后,对ELAN业务和系统内存的影响,[expr {($flag6_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA7 == [expr {($flag7_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，NE1($gpnIp1)反复进行保存设备重启后,对ELAN业务和系统内存的影响,[expr {($flag7_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA8 == [expr {($flag8_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，反复删增NE1($gpnIp1)的pw和ac的配置,对ELAN业务和系统内存的影响,[expr {($flag8_case1 == 1) ? "成功" : "失败"}]" $fileId
	gwd::GWpublic_print "== FA9 == [expr {($flag9_case1 == 1) ? "NOK" : "OK"}]" "（结论）满配ac时，导出NE1和NE2的配置，清空配置后恢复配置,对ELAN业务和系统内存的影响,[expr {($flag9_case1 == 1) ? "成功" : "失败"}]" $fileId
	...
	gwd::GWpublic_printAbnormal $fileId $fd_log 1 "FLAGE" $startSeconds "运行异常：错误为$err" "" "GPN_PTN_009_2"
}
close $fileId
log_file -noappend
close $fd_log
...
if {$flagResult == 1} {
	file rename "report\\GPN_PTN_009_2_REPORT.txt" "report\\NOK_GPN_PTN_009_2_REPORT.txt"
	file rename "log\\GPN_PTN_009_2_LOG.txt" "log\\NOK_GPN_PTN_009_2_LOG.txt"
} else {
	file rename "report\\GPN_PTN_009_2_REPORT.txt" "report\\OK_GPN_PTN_009_2_REPORT.txt"
	file rename "log\\GPN_PTN_009_2_LOG.txt" "log\\OK_GPN_PTN_009_2_LOG.txt"
}
