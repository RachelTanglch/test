#!/bin/sh
# T4_GPN_PTN_ELAN_009_4.tcl \
exec tclsh "$0" ${1+"$@"}
#-----------------------------------------------------------------------------------------------------------------------------------
#测试目的：E-LAN业务容量及健壮性测试
# 系统中承载VSI的容量测试
# CASE3
# 1、NE1到NE2、NE3、NE4上创建ETREE业务。NE1为root。使用一条lsp，创建系统容量个
#    vpls，每个vpls中3个pw、一个ac，ac的vlan接口从vlan10开始递增，步长为1
#    NE2、NE3、NE4上也创建相应的vpls业务，每个lsp、pw、ac都开启性能统计
# 2、NE1发送smac、vid（vid从10开始）递增递增的广播和组播流数据流
# 预期结果：NE2、NE3、NE4能收到所有的数据流
# 3、NE1和NE2、NE3、NE4互发smac、dmac、vid（vid从10开始）递增递增的已知单播，字
#    长为random（64-1518），四台设备都发送1Marp攻击报文（reply和qurey）vid从10开
#    始递增；使NNI口流量达到800M
# 预期结果：每条流都能正常转发，只检查是否收到每条数据流不检查丢包
# 3、读取NE1系统资源（show system resource usage）反复shutdown/undo shut NE1板
#    卡的NNI口和UNI口（次数可以配置），每次shut undoshut后检查每条流的转发，只检
#    查是否收到每条数据流不检查丢包
# 预期结果：每次shut undoshut后，每条流都能正常转发（2）cpu的利用率误差在5%，用户
#     利用率的误差在1%，系统利用率的误差在1%。（3）查看每个lsp、pw、ac的性能统
#     计但不检查是否正确只是打印到log中
...
#     利用率的误差在1%，系统利用率的误差在1%。（3）查看每个lsp、pw、ac的性能统
#     计但不检查是否正确只是打印到log中
# 5、读取系统资源（show system resource usage）反复进行NE1的交换盘主备倒换（次数
...
if {[catch {
	close stdout
	file mkdir "log"
	set fd_log [open "log\\GPN_PTN_009_4_LOG.txt" a]
	set stdout $fd_log
	chan configure $fd_log -blocking 0 -buffering none;#非阻塞模式 按行flush
	log_file log\\GPN_PTN_009_4_LOG.txt
	
	file mkdir "report"
	set fileId [open "report\\GPN_PTN_009_4_REPORT.txt" a+]
	chan configure $fileId -blocking 0 -buffering none;#非阻塞模式 按行flush
	
	file mkdir "debug"
	set fd_debug [open debug\\GPN_PTN_009_4_DEBUG.txt a]
	exp_internal -f debug\\GPN_PTN_009_4_DEBUG.txt 0
   ...
   ###配置端口1
	...
	foreach hStream "$dStreamData12 $dStreamData13 $dStreamData14 $dStreamData21 $dStreamData31 $dStreamData41" {
	...
	stc::apply
    incr tcId 
    gwd::GWpublic_saveTCCfg 1 $fd_log "GPN_LSP_009_4_$tcId.xml" [pwd]/Untitled
   #配置数据流量
   #用于导出被测设备用到的变量------
   ...
   set flag1_Case3 0;set flag2_Case3 0;set flag3_Case3 0;set flag4_Case3 0;set flag5_Case3 0;set flag6_Case3 0;set flag7_Case3 0;set flag8_Case3 0;set flag9_Case3 0;set flag10_Case3 0
	...
	for {set id 1} {$id <= $acPwCnt} {incr id} {
		foreach telnet $lSpawn_id matchType $lMatchType Gpn_type $lDutType {
			gwd::GWVpls_AddInfo $telnet $matchType $Gpn_type $fileId "vpls$id" $id "elan" "flood" "false" "false" "false" "true" "2000" "" "tagged"
		}
		...
		lappend flag1_Case3 [gwd::GWpublic_CfgPw $telnet2 $matchType2 $Gpn_type2 $fileId "$pw21" "vpls" "vpls$id" "peer" $address621 "$remote_label_1" "$local_label_1" "" "add" "none" 1 0 "0x8100" "0x8100" "1"]
		...
		lappend flag1_Case3 [gwd::GWpublic_CfgPw $telnet3 $matchType3 $Gpn_type3 $fileId "$pw31" "vpls" "vpls$id" "peer" $address631 "$remote_label_2" "$local_label_2" "" "add" "none" 1 0 "0x8100" "0x8100" "1"]
		...
		lappend flag1_Case3 [gwd::GWpublic_CfgPw $telnet4 $matchType4 $Gpn_type4 $fileId "$pw41" "vpls" "vpls$id" "peer" $address641 "$remote_label_3" "$local_label_3" "" "add" "none" 1 0 "0x8100" "0x8100" "1"]
		...
		lappend flag1_Case3 [gwd::GWAc_AddVplsInfo $telnet1 $matchType1 $Gpn_type1 $fileId "ac$acVlan" "vpls$id" "ethernet" $GPNTestEth1 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" ""]	
		lappend flag1_Case3 [gwd::GWAc_AddVplsInfo $telnet2 $matchType2 $Gpn_type2 $fileId "ac$acVlan" "vpls$id" "ethernet" $GPNTestEth2 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" ""]
		lappend flag1_Case3 [gwd::GWAc_AddVplsInfo $telnet3 $matchType3 $Gpn_type3 $fileId "ac$acVlan" "vpls$id" "ethernet" $GPNTestEth3 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" ""]	
		lappend flag1_Case3 [gwd::GWAc_AddVplsInfo $telnet4 $matchType4 $Gpn_type4 $fileId "ac$acVlan" "vpls$id" "ethernet" $GPNTestEth4 "$acVlan" "0" "root" "modify" "$acVlan" "0" "0" "0x8100" ""]
	...
	gwd::GW_AddUploadFile $telnet1 $matchType1 $Gpn_type1 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_009_4_fullvsiNE1.txt" "" "120"
	...
	gwd::GW_AddUploadFile $telnet2 $matchType2 $Gpn_type2 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_009_4_fullvsiNE2.txt" "" "120"
	...
	gwd::GW_AddUploadFile $telnet3 $matchType3 $Gpn_type3 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_009_4_fullvsiNE3.txt" "" "120"
	...
	gwd::GW_AddUploadFile $telnet4 $matchType4 $Gpn_type4 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_009_4_fullvsiNE4.txt" "" "120"
    incr cfgId
	lappend FLAGF [gwd::GWpublic_uploadDevCfg 1 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_4" lFailFileTmp]
	if {$lFailFileTmp != ""} {
    ...
    lappend flag2_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C2_2"]
	puts $fileId ""
	gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_009_4" lFailFileTmp
	if {"1" in $flag2_Case3} {
		set flagCase3 1
	...
	foreach eth "$GPNPort5 $GPNTestEth1" {
		...
		after $WaiteTime
	    	...
	    	lappend flag3_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C2_$j"]
	...
	foreach slot $rebootSlotlist2 {
		...
		lappend flag4_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C3_$j"]
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitchSw $telnet1 $matchType1 $Gpn_type1 $fileId]} {
			...
			after 120000
		       set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	    set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
	    lappend flag5_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C5_$j"]
		} else {
	       set testFlag 1
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitch $telnet1 $matchType1 $Gpn_type1 $fileId]} {
			...
			after 120000
		       set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	    set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
	    lappend flag6_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C6_$j"]
		} else {
	       set testFlag 1
	...
	for {set j 1} {$j<=$cnt} {incr j} {
		...
		after [expr 2*$rebootTime]
	       set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
	lappend flag7_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C7_$j"]
	...
	for {set j 1} {$j<=$cnt} {incr j} {
		...
		for {set k 1} {$k<11} {incr k} {
			lappend flag8_Case3 [gwd::GWVpls_AddInfo $telnet1 $matchType1 $Gpn_type1 $fileId "vpls$k" $k "elan" "flood" "false" "false" "false" "true" "2000" "" ""]
			lappend flag8_Case3 [gwd::GWpublic_CfgPw $telnet1 $matchType1 $Gpn_type1 $fileId "pw12_$k" "vpls" "vpls$k" "peer" $address612 "[expr 100+$k]" "[expr 100+$k]" "" "add" "none" 1 0 "0x8100" "0x8100" "2"]
			lappend flag8_Case3 [gwd::GWpublic_addPwStatEn $telnet1 $matchType1 $Gpn_type1 $fileId "pw12_$k" "enable"]
			lappend flag8_Case3 [gwd::GWpublic_CfgPw $telnet1 $matchType1 $Gpn_type1 $fileId "pw13_$k" "vpls" "vpls$k" "peer" $address612 "[expr 2100+$k]" "[expr 2100+$k]" "" "add" "none" 1 0 "0x8100" "0x8100" "3"]
			lappend flag8_Case3 [gwd::GWpublic_addPwStatEn $telnet1 $matchType1 $Gpn_type1 $fileId "pw13_$k" "enable"]
			lappend flag8_Case3 [gwd::GWpublic_CfgPw $telnet1 $matchType1 $Gpn_type1 $fileId "pw14_$k" "vpls" "vpls$k" "peer" $address612 "[expr 4100+$k]" "[expr 4100+$k]" "" "add" "none" 1 0 "0x8100" "0x8100" "4"]
	    ...
	    lappend flag8_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C8_$k"]
	...
	gwd::GWmanage_ftpDownload $telnet1 $matchType1 $Gpn_type1 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_009_4_fullvsiNE1.txt" ""
	gwd::GWmanage_ftpDownload $telnet2 $matchType2 $Gpn_type2 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_009_4_fullvsiNE2.txt" ""
	gwd::GWpublic_Reboot $telnet1 $matchType1 $Gpn_type1 $fileId
	gwd::GWpublic_Reboot $telnet2 $matchType2 $Gpn_type2 $fileId
	after [expr 2*$rebootTime]
	set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	...
	set telnet2 [gwd::GWpublic_loginGpn $gpnIp2 $userName2 $password2 $matchType2 $Gpn_type2 $fileId]
	set lSpawn_id [lreplace $lSpawn_id 1 1 $telnet2]
	lappend flag9_Case3 [MPLS_ClassStatisticsAna4 "1" $infoObj1 $infoObj2 $infoObj3 $infoObj4 "" "" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" "1000" "00:00:00:00:00:01" valuedrop1 valuedrop2 valuedrop3 valuedrop4 valuecnt1 valuecnt2 valuecnt3 valuecnt4 "GPN_PTN_009_4_C9"]
	...
	after $rebootTime
	gwd::GWpublic_printAbnormal $fileId $fd_log $flagDel "FLAGD" $startSeconds "测试结束后配置恢复" "测试结束后配置恢复" "GPN_PTN_009_4"
	gwd::GWpublic_printAbnormal $fileId $fd_log $FLAGF "FLAGF" $startSeconds $lFailFile "测试过程中所有配置文件都上传成功" "GPN_PTN_009_4"
	chan seek $fileId 0
	puts $fileId "\n**************************************************************************************\n"
	puts $fileId "**************************************************************************************\n"
	puts $fileId "GPN_PTN_009_4测试目的：E-LAN容量及稳定性测试\n"
	...
	gwd::GWpublic_print [expr {($flagResult == 1) ? "NOK" : "OK"}] "GPN_PTN_009_4测试结果" $fileId
	puts $fileId ""
	...
	gwd::GWpublic_printAbnormal $fileId $fd_log 1 "FLAGE" $startSeconds "运行异常：错误为$err" "" "GPN_PTN_009_4"
}
close $fileId
log_file -noappend
close $fd_log
...
if {$flagResult == 1} {
	file rename "report\\GPN_PTN_009_4_REPORT.txt" "report\\NOK_GPN_PTN_009_4_REPORT.txt"
	file rename "log\\GPN_PTN_009_4_LOG.txt" "log\\NOK_GPN_PTN_009_4_LOG.txt"
} else {
	file rename "report\\GPN_PTN_009_4_REPORT.txt" "report\\OK_GPN_PTN_009_4_REPORT.txt"
	file rename "log\\GPN_PTN_009_4_LOG.txt" "log\\OK_GPN_PTN_009_4_LOG.txt"
}
