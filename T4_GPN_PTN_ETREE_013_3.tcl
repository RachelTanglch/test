#!/bin/sh
# T4_GPN_PTN_ETREE_013_3.tcl \
exec tclsh "$0" ${1+"$@"}
# 1个VSI承载1个AC和
# 系统容量-1个PW的容量测试   
# Test Case 2
# vpls中配置ac和pw的容量是一个可设置的值，设这个值为acPwCnt
# 1、NE1和NE2之间创建E-Tree业务
#    NE1上一个vpls域、一个LSP，一个ac为port+10,acPwCnt-1个pw到NE2；
#    NE2上一个vpls域、一个LSP，一个ac为port+10,acPwCnt-1个pw到NE1
#    在每个lsp、pw、ac上使能性能统计
# 2、NE1和NE2发送vid=10的未知单播xM
# 预期结果：（1）收到（acPwCnt-1）*xM，不检查丢包
# 3、停止发流，查看NE1的NNI口单播包的统计个数
# 预期结果：统计个数正确，与TC的收发比较
# 4、在NE1和ne2的UNI口相互发送满带宽的已知单播流600M（字节长度为random，len从64-
#    1518）；多种报文为协议报0.5M；arp攻击报文0.5M
# 预期结果：已知单播不丢包、协议报文不丢包、arp报文不丢包
# 3、读取系统资源（show system resource usage）反复shutdown/undo shut NE1板卡的
#    NNI口和UNI口（次数可以配置）
...
# 4、读取系统资源（show system resource usage）反复重启NE1的NNI口和UNI口所在板卡
#   （次数可以配置）
# 预期结果：每次重启板卡后检查
# （1）已知单播不丢包、协议报文不丢包、arp报文不丢包
# （2）show interface pw 和show interface ac
...
# （4）show task的结果打印到log中
# （5）cpu的利用率误差在1%，用户利用率的误差在0.1%，系统利用率的误差在0.1%
# 8、反复删增10个ac和pw的配置，50次，要求次数可配置，每次删增后检查
# （1）已知单播不丢包、协议报文不丢包、arp报文不丢包
# （2）show interface pw 和show interface ac
# （3）查看NE1 lsp ac pw的性能统计并把结果打印到log中，不判断性能统计是否正确
# （4）show task的结果打印到log中
# （5）cpu的利用率误差在1%，用户利用率的误差在0.1%，系统利用率的误差在0.1%。
# 8、导出NE1和NE2的配置，清空配置后重启
# 预期结果：在NE1和NE4上通过show running mpls查看，无etree配置
# 9、导入第8步导出的NE1和NE1的配置并重启NE1和NE1，重启后检查业务和系统资源
# 预期结果：
# （1）已知单播不丢包、协议报文不丢包、arp报文不丢包
# （2）show interface pw 和show interface ac
# （3）查看NE1 lsp ac pw的性能统计并把结果打印到log中，不判断性能统计是否正确
...
if {[catch {
	close stdout
	file mkdir "log"
	set fd_log [open "log\\GPN_PTN_013_3_LOG.txt" a]
	set stdout $fd_log
	chan configure $fd_log -blocking 0 -buffering none;#非阻塞模式 按行flush
	log_file log\\GPN_PTN_013_3_LOG.txt
	
	file mkdir "report"
	set fileId [open "report\\GPN_PTN_013_3_REPORT.txt" a+]
	chan configure $fileId -blocking 0 -buffering none;#非阻塞模式 按行flush
	
	file mkdir "debug"
	set fd_debug [open debug\\GPN_PTN_013_3_DEBUG.txt a]
	exp_internal -f debug\\GPN_PTN_013_3_DEBUG.txt 0
   ...
   set RangeModifier212 [stc::create "RangeModifier" \
	...
	###配置vpls
	lappend flag1_Case2 [gwd::GWVpls_AddInfo $telnet1 $matchType1 $Gpn_type1 $fileId "vpls1" 1 "etree" "flood" "false" "false" "false" "true" "5000" "" "tagged"]
	lappend flag1_Case2 [gwd::GWVpls_AddInfo $telnet2 $matchType2 $Gpn_type2 $fileId "vpls1" 1 "etree" "flood" "false" "false" "false" "true" "5000" "" "tagged"]
    ####配置NE1
	...
	lappend flag1_Case2 [gwd::GWpublic_CfgPw $telnet2 $matchType2 $Gpn_type2 $fileId "$pw21" "vpls" "vpls1" "peer" $address621 "100" "100" "" "add" "root" 1 0 "0x8100" "0x8100" "1"]
	####配置lsp、ac、pw性能统计使能
	...
	lappend flag1_Case2 [gwd::GWAc_AddVplsInfo $telnet1 $matchType1 $Gpn_type1 $fileId "ac1000" "vpls1" "ethernet" $GPNTestEth1 "1000" "0" "none" "modify" "1000" "0" "0" "0x8100" "evc2"]	
	lappend flag1_Case2 [gwd::GWAc_AddVplsInfo $telnet2 $matchType2 $Gpn_type2 $fileId "ac1000" "vpls1" "ethernet" $GPNTestEth2 "1000" "0" "leaf" "modify" "1000" "0" "0" "0x8100" "evc2"]
	...
	for {set i 1} {$i < [expr $acPwCnt-1]} {incr i} {
		...
		lappend flag1_Case2 [gwd::GWpublic_CfgPw $telnet2 $matchType2 $Gpn_type2 $fileId "$pw21" "vpls" "vpls1" "peer" $address621 "$local_label" "$remote_label" "" "add" "root" 1 0 "0x8100" "0x8100" "1"]
		set getCnt 3
	...
	gwd::GW_AddUploadFile $telnet1 $matchType1 $Gpn_type1 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_013_3_fullpwNE1.txt" "" "120"
	...
	gwd::GW_AddUploadFile $telnet2 $matchType2 $Gpn_type2 $fileId [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"config" "GPN_PTN_013_3_fullpwNE2.txt" "" "120"
     if {"1" in $flag1_Case2} {
		set flagCase2 1
    ...
    foreach hStream "$dStreamData11 $dStreamData21" {
		...
		stc::config [stc::get $hStream -AffiliationStreamBlockLoadProfile-targets] -load 0.01 -LoadUnit MEGABITS_PER_SECOND
	...
	foreach hStream "$dStreamData11 $dStreamData21" {
	   ...
	   stc::apply
	   incr tcId
    gwd::GWpublic_saveTCCfg 1 $fd_log "GPN_LSP_013_2_Case2_$tcId.xml" [pwd]/Untitled
	   send_log "\n 开始:[clock format [clock seconds] -format "%Y-%m-%d,%H:%M:%S"]"
    lappend flag2_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B2_2"]
	   if {"1" in $flag2_Case2} {
		set flagCase2 1
	...
	puts $fileId ""
	gwd::GWpublic_uploadDevCfg 0 $lSpawn_id $lMatchType $lDutType $fileId $fd_log $startSeconds $lIp $ftp $cfgId "GPN_PTN_013_3" lFailFileTmp
	...
	foreach eth "$GPNPort5 $GPNTestEth1" {
	    ...
	    lappend flag3_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B3_$j"]
	...
	foreach slot $rebootSlotlist2 {
		...
		lappend flag4_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B4_$j"]
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitchSw $telnet1 $matchType1 $Gpn_type1 $fileId]} {
			...
			after 120000
		       set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	    set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
	    lappend flag5_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B5_$j"]
		} else {
	       set testFlag 1
	...
	for {set j 1} {$j<=$cntdh} {incr j} {
		...
		if {![gwd::GWCard_AddSwitch $telnet1 $matchType1 $Gpn_type1 $fileId]} {
			...
			after 120000
		       set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	    set lSpawn_id [lreplace $lSpawn_id 0 0 $telnet1]
	    lappend flag6_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B6_$j"]
		} else {
	       set testFlag 1
	...
	lappend flag7_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B7_$j"]
	...
	stc::apply
	    lappend flag8_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B8_$j"]
	...
	gwd::GWmanage_ftpDownload $telnet1 $matchType1 $Gpn_type1 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_013_3_fullpwNE1.txt" ""
	gwd::GWmanage_ftpDownload $telnet2 $matchType2 $Gpn_type2 $fileId $fd_log 60 "config" [dict get $ftp ip] [dict get $ftp userName] [dict get $ftp passWord] \
		"GPN_PTN_013_3_fullpwNE2.txt" ""
	gwd::GWpublic_Reboot $telnet1 $matchType1 $Gpn_type1 $fileId
	gwd::GWpublic_Reboot $telnet2 $matchType2 $Gpn_type2 $fileId
	after [expr 2*$rebootTime]
	set telnet1 [gwd::GWpublic_loginGpn $gpnIp1 $userName1 $password1 $matchType1 $Gpn_type1 $fileId]
	...
	stc::apply
	lappend flag9_Case2 [MPLS_ClassStatisticsPort $fileId $hGPNPort1Ana $hGPNPort1Gen $hGPNPort2Ana $hGPNPort2Gen "GPN_PTN_013_2_B9_$j"]
	...
	after $rebootTime
	gwd::GWpublic_printAbnormal $fileId $fd_log $flagDel "FLAGD" $startSeconds "测试结束后配置恢复" "测试结束后配置恢复" "GPN_PTN_013_3"
	gwd::GWpublic_printAbnormal $fileId $fd_log $FLAGF "FLAGF" $startSeconds $lFailFile "测试过程中所有配置文件都上传成功" "GPN_PTN_013_3"
	chan seek $fileId 0
	puts $fileId "\n**************************************************************************************\n"
	puts $fileId "**************************************************************************************\n"
	puts $fileId "GPN_PTN_013_3测试目的：ETREE容量及稳定性测试\n"
	...
	gwd::GWpublic_print [expr {($flagResult == 1) ? "NOK" : "OK"}] "GPN_PTN_013_3测试结果" $fileId
	puts $fileId ""
	...
	gwd::GWpublic_printAbnormal $fileId $fd_log 1 "FLAGE" $startSeconds "运行异常：错误为$err" "" "GPN_PTN_013_3"
}
close $fileId
log_file -noappend
close $fd_log
...
if {$flagResult == 1} {
	file rename "report\\GPN_PTN_013_3_REPORT.txt" "report\\NOK_GPN_PTN_013_3_REPORT.txt"
	file rename "log\\GPN_PTN_013_3_LOG.txt" "log\\NOK_GPN_PTN_013_3_LOG.txt"
} else {
	file rename "report\\GPN_PTN_013_3_REPORT.txt" "report\\OK_GPN_PTN_013_3_REPORT.txt"
	file rename "log\\GPN_PTN_013_3_LOG.txt" "log\\OK_GPN_PTN_013_3_LOG.txt"
}
